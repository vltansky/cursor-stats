...
import { boxHeader, boxDivider, boxFooter, boxLine, emptyLine, sectionTitle, createProgressBar, padRight } from '../lib/formatters.js';
export function displayVibeStats(stats) {
    console.clear();
    const lines = [];
    const width = 74;
    // Header
...
function getThinkingModeEmoji(mode) {
    const emojis = {
        deep: '🧠',
        speed: '⚡',
        experimental: '🧪',
...
function getPolitenessBadge(score) {
    if (score >= 80)
        return '😇';
    if (score >= 60)
        return '🙂';
...
function getCommunicationStyle(comm) {
    if (comm.politenessScore >= 70)
        return {
            name: 'Polite Partner',
            emoji: '😇',
            description: 'You treat AI with kindness and respect!'
        };
    if (comm.direct > comm.collaborative && comm.direct > comm.polite)
        return {
            name: 'The Boss',
            emoji: '👑',
            description: 'Direct and to the point - you know what you want!'
        };
    if (comm.collaborative > comm.direct && comm.collaborative > comm.polite)
        return {
            name: 'Team Player',
            emoji: '🤝',
