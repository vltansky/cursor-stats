...
function calculateEmotionalStats(userMessages) {
    ...
    const topEmotion = Object.entries(emotions).reduce((a, b) => a[1] > b[1] ? a : b, ['neutral', 0])[0];
...
function calculateLearningStats(userMessages) {
    const allText = userMessages.map(m => m.text).join(' ');
...
function calculateThinkingStats(userMessages) {
    const allText = userMessages.map(m => m.text).join(' ');
    ...
    const thinkingMode = total === 0 ? 'balanced' : Object.entries(modes).reduce((a, b) => a[1] > b[1] ? a : b)[0];
    return {
...
function calculateTaskStats(userMessages) {
    const allText = userMessages.map(m => m.text).join(' ');
...
function calculateConfidenceStats(userMessages) {
    const allText = userMessages.map(m => m.text).join(' ');
...
function calculateCommunicationStats(userMessages) {
    const allText = userMessages.map(m => m.text).join(' ');
